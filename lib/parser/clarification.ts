...
function questionForIssue(issue: ParserValidationIssue): string {
  ...
  if (issue.code === "allocation_required") {
    return `${issue.message} Choose one: split the total equally between items, enter item amounts manually, or treat it as one combined shared item and remove item-level exclusions.`;
  }
